...
class _SeriesPageState extends State<SeriesPage> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    UiPrefs.seriesMinBooks.addListener(_onMinBooksChanged);
    _startConnectivityWatch();
    _loadViewTypePref().then(
      (_) => _loadSearchPref().then((_) => _refresh(initial: true)),
    );
  }
  ...
  void didChangeDependencies() {
    ...
    _completionSub ??= ServicesScope.of(
      context,
    ).services.playback.completionStatusStream.listen((event) {
  ...
  void dispose() {
    ...
    UiPrefs.seriesMinBooks.removeListener(_onMinBooksChanged);
    ...
    String normalized =
        seriesName
            ...
            .replaceAll(
              RegExp(r'\s*Book\s+\d+'),
              '',
            ) // Remove "Book 1", "Book 2", etc.
            .replaceAll(
              RegExp(r'\s*Volume\s+\d+'),
              '',
            ) // Remove "Volume 1", etc.
            .replaceAll(RegExp(r'\s*Part\s+\d+'), '') // Remove "Part 1", etc.
            .replaceAll(
              RegExp(r'\s*Episode\s+\d+'),
              '',
            ) // Remove "Episode 1", etc.
            .replaceAll(
              RegExp(r'\s*Chapter\s+\d+'),
              '',
            ) // Remove "Chapter 1", etc.
            .replaceAll(
              RegExp(r'\s*Season\s+\d+'),
              '',
            ) // Remove "Season 1", etc.
            .replaceAll(
              RegExp(r'\s*Series\s+\d+'),
              '',
            ) // Remove "Series 1", etc.
            .replaceAll(
              RegExp(r'\s*Trilogy\s+\d+'),
              '',
            ) // Remove "Trilogy 1", etc.
            .replaceAll(RegExp(r'\s*Saga\s+\d+'), '') // Remove "Saga 1", etc.
            .replaceAll(
              RegExp(r'\s*Chronicles\s+\d+'),
              '',
            ) // Remove "Chronicles 1", etc.
            .replaceAll(
              RegExp(r'\s*Diaries\s+\d+'),
              '',
            ) // Remove "Diaries 1", etc.
            .replaceAll(RegExp(r'\s*Files\s+\d+'), '') // Remove "Files 1", etc.
            .replaceAll(
              RegExp(r'\s*Sequence\s+\d+'),
              '',
            ) // Remove "Sequence 1", etc.
            .replaceAll(
              RegExp(r'\s*Universe\s+\d+'),
              '',
            ) // Remove "Universe 1", etc.
            .replaceAll(
              RegExp(r'\s*Collection\s+\d+'),
              '',
            ) // Remove "Collection 1", etc.
            .replaceAll(RegExp(r'\s*Cycle\s+\d+'), '') // Remove "Cycle 1", etc.
            .replaceAll(RegExp(r'\s*Saga\s*$'), '') // Remove trailing "Saga"
            .replaceAll(
              RegExp(r'\s*Trilogy\s*$'),
              '',
            ) // Remove trailing "Trilogy"
            .replaceAll(
              RegExp(r'\s*Chronicles\s*$'),
              '',
            ) // Remove trailing "Chronicles"
            .replaceAll(
              RegExp(r'\s*Diaries\s*$'),
              '',
            ) // Remove trailing "Diaries"
            .replaceAll(RegExp(r'\s*Files\s*$'), '') // Remove trailing "Files"
            .replaceAll(
              RegExp(r'\s*Sequence\s*$'),
              '',
            ) // Remove trailing "Sequence"
            .replaceAll(
              RegExp(r'\s*Universe\s*$'),
              '',
            ) // Remove trailing "Universe"
            .replaceAll(
              RegExp(r'\s*Collection\s*$'),
              '',
            ) // Remove trailing "Collection"
  ...
  List<Series> _filterSeriesData(List<Series> data) {
    ...
    // Hide series with fewer than the configured number of books (1 = off).
    final minBooks = UiPrefs.seriesMinBooks.value;
    if (minBooks > 1) {
      result = result.where((s) => s.numBooks >= minBooks).toList();
    }

    final query = _query.trim().toLowerCase();
    if (query.isEmpty) return result;
    return result.where((series) {
    ...
    try {
      ...
      try {
        ...
        setState(() {
          _loading = false;
          _error =
              'Failed to load series: $e. Fallback also failed: $fallbackError';
        });
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filteredSeries =
        _viewType == SeriesViewType.series
            ? _filterSeriesData(_series)
            : <Series>[];
    final filteredCollections =
        _viewType == SeriesViewType.collections
            ? _filterCollectionsData(_collections)
            : <String, List<Book>>{};
            ...
            slivers: [
              SliverAppBar(
                ...
                actions: [
                  ...
                  // Series-only options (e.g. hide small series)
                  if (_viewType == SeriesViewType.series) ...[
                    AppLiquidGlassPill(
                      padding: const EdgeInsets.all(4),
                      blur: 34,
                  ...
                  sliver:
                      _viewType == SeriesViewType.series
                          ...
                          : SliverList.builder(
                            ...
                            itemBuilder: (context, i) {
                              final entry = filteredCollections.entries
                                  .elementAt(i);
...
class _FannedCover extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final layers = <Widget>[];
    // Fanned covers, farthest painted first so nearer ones overlap them.
    for (var i = extra.length - 1; i >= 0; i--) {
      final step = i + 1; // 1 or 2
      layers.add(Positioned(
        left: spread * step / extra.length,
        top: 0,
        child: Transform.rotate(
          angle: 0.085 * step, // clockwise; fans to the right
            ...
            child: Stack(
              children: [
                ...
                Positioned.fill(
                  child: IgnorePointer(
                    child: Container(
                      color: Colors.black.withOpacity(0.18 + 0.12 * i),
                    ),
                  ),
                ),
              ],
...
class _SeriesBookCard extends StatelessWidget {
  const _SeriesBookCard({required this.book, required this.onTap});
...
class _BookStatusIndicator extends StatelessWidget {
  const _BookStatusIndicator({required this.book, this.compact = false});
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<_SeriesBookStatus>(
      ...
      builder: (context, snapshot) {
        ...
        return _BookStatusBadge(status: snapshot.data!, compact: compact);
      },
    );
  }
}
...
class _BookStatusBadge extends StatelessWidget {
  const _BookStatusBadge({required this.status, required this.compact});
...
class _SeriesBookStatusResolver {
  static final Map<String, _CachedBookStatus> _cache =
      <String, _CachedBookStatus>{};
  static final Map<String, Future<_SeriesBookStatus>> _inFlight =
      <String, Future<_SeriesBookStatus>>{};
  ...
  static Future<_SeriesBookStatus> resolve(AppServices services, Book book) {
    ...
    final future = _fetchStatus(services, book)
        .then((status) {
  ...
  static Future<_SeriesBookStatus> _fetchStatus(
  ...
  static Future<_ProgressSnapshot> _fetchProgressSnapshot(
    AppServices services,
    String bookId,
  ) async {
    try {
      final resp = await services.auth.api.request(
        'GET',
