...
class _CollectionsPageState extends State<CollectionsPage> {
  ...
  Future<void> _loadAll() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _collections = map; _loading = false; });
    } catch (e) {
      if (!mounted) return;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Failed to load collections', style: Theme.of(context).textTheme.titleMedium?.copyWith(color: cs.error)),
      ...
      return Center(
        child: Padding(
          ...
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(Icons.folder_outlined, size: 64, color: cs.onSurfaceVariant),
              const SizedBox(height: 16),
              Text('No collections yet', style: Theme.of(context).textTheme.titleLarge),
              const SizedBox(height: 8),
              Text('Collections appear when a group has 2 or more books', textAlign: TextAlign.center, style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: cs.onSurfaceVariant)),
            ],
          ),
        ),
      );
    }
    final keys = _collections.keys.toList()..sort((a, b) => a.toLowerCase().compareTo(b.toLowerCase()));
          ...
          SliverPadding(
            ...
            sliver: SliverList.builder(
              ...
              itemBuilder: (context, i) {
                final name = keys[i];
                final items = _collections[name]!;
                return _CollectionCard(name: name, books: items, onTapBook: (b) {
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(child: Text(name, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700))),
                Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: cs.surfaceContainerHighest, borderRadius: BorderRadius.circular(12)), child: Text('${books.length}', style: Theme.of(context).textTheme.labelMedium?.copyWith(color: cs.onSurfaceVariant))),
              ],
            ),
            const SizedBox(height: 8),
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, i) {
                  ...
                  if (uri != null && uri.scheme == 'file') {
                    ...
                    img = f.existsSync() ? Image.file(f, fit: BoxFit.cover) : const SizedBox.shrink();
                  } else {
                    img = Image.network(b.coverUrl, fit: BoxFit.cover);
                  }
                  return AspectRatio(
                    aspectRatio: 2/3,
                    child: InkWell(
