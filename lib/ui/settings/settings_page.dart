...
class _SettingsPageState extends State<SettingsPage> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Settings exported to $result')));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
    ...
    if (ModalRoute.of(context)?.isCurrent == true) {
      ...
      _reloadDebounce = Timer(const Duration(milliseconds: 300), () {
        ...
        if (_lastReloadTime == null ||
            now.difference(_lastReloadTime!) >
    ...
    try {
      ...
      setState(() {
        ...
        _pauseCancelsSleepTimer =
            prefs.getBool('pause_cancels_sleep_timer') ?? true;
        _dualProgressEnabled =
            prefs.getBool('ui_dual_progress_enabled') ?? true;
        _detailedPlayHistoryEnabled =
            prefs.getBool('detailed_play_history_enabled') ?? false;
        _resumeFromHistoryEnabled =
            prefs.getBool('ui_resume_from_history_enabled') ?? true;
        _confirmResumeFromHistory =
    ...
    try {
      ...
      for (final it in list) {
        if (it is Map) {
          ...
          final mt =
              (m['mediaType'] ?? m['type'] ?? '').toString().toLowerCase();
          if (id.isNotEmpty)
            libs.add({'id': id, 'name': name, 'mediaType': mt});
        }
      }
      if (mounted)
        setState(() {
  ...
  Future<void> _exportLogFile() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Log exported to $result')));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
  ...
  Future<void> _switchLibrary(String newId) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Library switched')));
      }
    } catch (_) {}
  }
  ...
  Future<void> _updateStreamingCacheLimit(double valueMb) async {
    ...
    await StreamingCacheService.instance.setMaxBytes(
      (normalized.round()) * _mb,
    );
    await _refreshStreamingCacheUsage();
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Streaming cache capped at ${_formatMbLabel(normalized)}',
        ),
      ),
    );
  }
  ...
  Future<void> _clearStreamingCache() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Streaming cache cleared')));
    }
  }
  ...
  Widget _buildStreamingCacheSubtitle(BuildContext context) {
    ...
    final usageLabel =
        usage == null
            ? 'Calculating usage…'
            : 'Currently using ${_formatBytes(usage)}';
  ...
  Future<void> _performCleanup() async {
    ...
    showDialog(
      ...
      builder:
          (context) => _CleanupProgressDialog(
            key: dialogKey,
            onCancel: () {
              cancelled = true;
    ...
    try {
      ...
      if (mounted && !cancelled) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Cleanup cancelled. $cleanedUp items were cleaned up before cancellation.',
            ),
            duration: const Duration(seconds: 3),
          ),
        );
    ...
    try {
      ...
      if (mounted && !cancelled) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Resync cancelled. $totalSynced books were synced before cancellation.',
            ),
            duration: const Duration(seconds: 3),
          ),
        );
        ...
        body: ListView(
          children: [
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Show a dedicated Authors tab in the main navigation',
              ),
              value: _authorViewEnabled ?? true,
              onChanged: (v) async {
                await UiPrefs.setAuthorViewEnabled(
                  v,
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Hide series name in books list when it matches the author name',
              ),
              value: _hideSeriesWhenSameAsAuthor ?? true,
              onChanged: (v) async {
                await UiPrefs.setHideSeriesWhenSameAsAuthor(
                  v,
                  pinToSettingsOnChange: true,
                );
                if (mounted)
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Apply a gradient surface to the full screen player',
              ),
              value: UiPrefs.playerGradientBackground.value,
              onChanged: (v) async {
                await UiPrefs.setPlayerGradientBackground(
                  v,
                  pinToSettingsOnChange: true,
                );
                if (mounted) setState(() {});
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Show one title line that continuously scrolls left in full player',
              ),
              value: _playerScrollingSingleLineTitle ?? false,
              onChanged: (v) async {
                await UiPrefs.setPlayerScrollingSingleLineTitle(
                  v,
                  pinToSettingsOnChange: true,
                );
                if (mounted)
                  setState(() => _playerScrollingSingleLineTitle = v);
              },
            ),
            SwitchListTile(
              title: const Text('Add Letter Scrolling'),
              subtitle: const Text(
                'Show an alphabetical scrollbar in long lists',
              ),
              value: _letterScrollEnabled ?? false,
              onChanged: (v) async {
                await UiPrefs.setLetterScrollEnabled(
                  v,
                  pinToSettingsOnChange: true,
                );
                if (mounted)
                  setState(() {
                    _letterScrollEnabled = v;
                  });
              },
            ),
            Padding(
              ...
              child: SwitchListTile(
                ...
                subtitle: const Text(
                  'Required for letter scrolling in the Books tab',
                ),
                value: _letterScrollBooksAlpha ?? false,
                onChanged:
                    (_letterScrollEnabled ?? false)
                        ? (v) async {
                          await UiPrefs.setLetterScrollBooksAlpha(
                            v,
                            pinToSettingsOnChange: true,
                          );
                          if (mounted)
                            setState(() {
                              _letterScrollBooksAlpha = v;
                            });
                        }
                        : null,
              ),
            ...
            ValueListenableBuilder<ThemeMode>(
              ...
              builder: (_, mode, __) {
                ...
                return SwitchListTile(
                  ...
                  value: isDark,
                  onChanged:
                      (v) => theme.set(v ? ThemeMode.dark : ThemeMode.light),
              ...
              builder: (_, tintLevel, __) {
                return ListTile(
                  ...
                  trailing: DropdownButton<SurfaceTintLevel>(
                    value: tintLevel,
                    items:
                        SurfaceTintLevel.values.map((level) {
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Disable to allow downloads on cellular data',
              ),
            ...
            ListTile(
              ...
              trailing:
                  (_streamingCacheLimitMb == null)
                      ? null
                      : TextButton(
                        onPressed:
                            _clearingStreamingCache
                                ? null
                                : _clearStreamingCache,
                        child:
                            _clearingStreamingCache
                                ? const SizedBox(
                                  width: 16,
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Rewind a few seconds based on pause duration',
              ),
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Fetch latest progress from server before starting playback',
              ),
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Stop the sleep timer when pausing playback',
              ),
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Show global book progress and chapter progress',
              ),
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Show “Resume previous play position” under the cover',
              ),
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Record play sessions for stats (top books/authors/narrators)',
              ),
            ...
            ListTile(
              ...
              subtitle: const Text(
                'Choose which progress bar is front and center (full player + notification)',
              ),
              trailing: DropdownButton<ProgressPrimary>(
                value: _progressPrimary ?? ProgressPrimary.book,
                onChanged: (value) async {
                  if (value == null) return;
                  await UiPrefs.setProgressPrimary(
                    value,
                    pinToSettingsOnChange: true,
                  );
                  if (!mounted) return;
                  setState(() {
                    _progressPrimary = value;
                  });
                },
                items: const [
                  DropdownMenuItem(
            ...
            Padding(
              ...
              child: Slider(
                ...
                onChanged: (value) {
                  ...
                  final rounded = validValues.reduce(
                ...
                onChangeEnd: (value) async {
                  ...
                  await UiPrefs.setSeekBackwardSeconds(
                    rounded,
                    pinToSettingsOnChange: true,
                  );
                  if (!mounted) return;
            ...
            Padding(
              ...
              child: Slider(
                ...
                onChangeEnd: (value) async {
                  ...
                  await UiPrefs.setSeekForwardSeconds(
                    rounded,
                    pinToSettingsOnChange: true,
                  );
                  if (!mounted) return;
            ...
            SwitchListTile(
              ...
              subtitle: const Text(
                'Start playing when connected to car Bluetooth',
              ),
            ...
            SwitchListTile(
              ...
              subtitle:
                  _loggingSessionActive
                      ...
                      : const Text(
                        'Log everything to file for up to 15 minutes',
                      ),
...
class _CleanupProgressDialog extends StatefulWidget {
  const _CleanupProgressDialog({super.key, required this.onCancel});
